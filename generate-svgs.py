...
outputDir = os.path.join(os.path.dirname(__file__), 'public', 'images')
os.makedirs(outputDir, exist_ok=True)

def getGradient(i):
    hues = [120, 200, 340, 45, 270, 15]
    h = hues[i % len(hues)]
    return f"""<linearGradient id="grad{i}" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" style="stop-color:hsl({h}, 40%, 30%);stop-opacity:1" />
      <stop offset="100%" style="stop-color:hsl({h+30}, 50%, 15%);stop-opacity:1" />
    </linearGradient>"""

i = 0
for key, title in missingProducts.items():
  ...
  <text x="400" y="380" font-family="system-ui, -apple-system, sans-serif" font-size="80" fill="#ffffff" text-anchor="middle" font-weight="bold" opacity="0.8">{title[0]}</text>
  ...
  <text x="400" y="630" font-family="system-ui, -apple-system, sans-serif" font-size="24" fill="#ffffff" text-anchor="middle" opacity="0.6">Premium Quality</text>
    ...
    filepath = os.path.join(outputDir, f"{key}.svg")
    with open(filepath, 'w') as f:
        f.write(svg)
    i += 1

print('Successfully generated SVG placeholders.')
